...
"SVM"
def SVM(kernel,train_data,train_label,issmote='no_smote',neighbors=5):
    if issmote=="smote":
        train_data,train_label=smote_most(train_data,train_label,k=neighbors)
    elif issmote=="border":
        train_data,train_label=smote_border(train_data,train_label,k=neighbors)
...
def str_clean(body):
    return re.sub(r"<(.*?)>|\n|(\\(.*?){)|}|[!$%^&*()_+|~\-={}\[\]:\";'<>?,.\/\\]|[0-9]|[@]",' ', body.lower()).encode('ascii','ignore')
...
def active_learning(data,label,pool,Classify=do_SVM,sampling="uncertainty",issmote="no_smote",neighbors=5,step=50,last=10):
    ...
    for i in x:
        ...
        if sampling=="uncertainty":
            ...
            certainty = [1-x[list(clf.classes_).index("pos")] for x in prob]
        else:
